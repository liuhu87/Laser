{
   ...
   double elevation[6]={10,20,30,40,50,55};
   ...
   //pad1->SetLogy(1);
   //pad1->cd();
   TFile* fin=TFile::Open(Form("/eos/user/h/hliu/LaserEvent/tel%d_corr.root",iTel));
   ...
   int color[]={2,2,3,4,6,9};
   int cindex=5;
   int nplot=0;
   for(int iele=0;iele<6;iele++){
      if(cindex>=0&&iele!=cindex) continue;
      TH2F* hist=(TH2F*)fin->Get(Form("Tel%d_Ele%d_corr",iTel,iele+1));
      if(!hist) continue;
      ...
      leg->AddEntry(gr[nplot],Form("iTel=%d Elevation=%.0lf",iTel,elevation[iele]),"l");
