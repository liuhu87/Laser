...
int main(int argc,char* argv[]){
   ...
   for(int ientry=firstevent;ientry<maxevent;ientry++){
      ...
      pev->DoFit(0,3);
      printf("Time=%d+%.10lf kk={%.2lf+-%.2lf} cc={%.2lf+-%.2lf}\n",pev->rabbitTime,pev->rabbittime*20*1.0e-9,pev->minimizer->X()[3]/PI*180,pev->minimizer->Errors()[3]/PI*180,pev->minimizer->X()[2],pev->minimizer->Errors()[2]);
   }
